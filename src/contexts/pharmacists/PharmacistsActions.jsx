...
export const updatePharmacist = async (data1, dispatch) => {
  ...
  try {
    const { data } = await axios.put(
      `https://arcane-garden-68747.herokuapp.com/api/pharmacists/${data1._id}/update`,
      data1,
      {
        headers: {
          authorization:
...
export const changeActive = async (id, dispatch) => {
  try {
    const { data } = await axios.put(
      `https://arcane-garden-68747.herokuapp.com/api/pharmacists/${id}/activeState`,
      null,
      {
        headers: {
          authorization:
...
export const createPrescription = async (data1, dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      `https://arcane-garden-68747.herokuapp.com/api/pharmacists/createPrescription`,
      data1,
      {
        headers: {
          authorization:
    ...
    console.log(error);
    dispatch({ type: 'CREATE_PRESCRIPTION_FAIL', payload: error });
  }
};
...
export const getTotalPriceInDate = async (data1, dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      `https://arcane-garden-68747.herokuapp.com/api/pharmacists/getPrescriptionFee`,
      data1,
      {
        headers: {
          authorization:
